...
plot(grp_snp_cov$DECSLON[which(year(grp_snp_cov$date)==2014)],
     grp_snp_cov$DECSLAT[which(year(grp_snp_cov$date)==2014)],
     xlim=range(grp_snp_cov$DECSLON),ylim=range(grp_snp_cov$DECSLAT),
     cex=grp_snp_cov$rg_pr_ab[which(year(grp_snp_cov$date)==2014)]+1,asp=1)

setwd('~/Desktop/professional/projects/Postdoc_FL/data/grouper/')
write.csv(grp_snp_cov,'grp_snp_2019.csv',row.names = F)
...
rg_lth_freq_n$date <- mdy_hm(paste(rg_lth_freq_n$MO_DAY_YR,
                                   paste(substr(sprintf('%04d',as.numeric(rg_lth_freq_n$TIME_MIL)),1,2),
                                         substr(sprintf('%04d',as.numeric(rg_lth_freq_n$TIME_MIL)),3,4),sep=':')))
